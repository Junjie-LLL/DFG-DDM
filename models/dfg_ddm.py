...
class DFG_UNet(nn.Module):
    def __init__(self, config):
        ...
        self.conv_out = torch.nn.Conv2d(block_in,
                                        ...
                                        stride=1,
                                        padding=1)
        self.fft1 = ResBlock_fft_bench(n_feat=128)
        self.fft2 = ResBlock_fft_bench(n_feat=512)
    ...
    def forward(self, x, t):
        ...
        # downsampling
        # swin=self.E_block(x)#32    
        # swin=self.PPM(swin)   # (1,64,256,256)
        # swin = self.conv0(swin)
        tt = self.conv_in(x)
        hs = [self.fft1(tt)]
        for i_level in range(self.num_resolutions):
            for i_block in range(self.num_res_blocks):
        ...
        # swin3=self.E_block3(h)#32    
        # swin3=self.PPM3(swin3)   # (1,64,256,256) 256,8
        # swin3 = self.conv2(swin3)
        h = self.fft1(h)
        # end
        h = self.norm_out(h)
        h = nonlinearity(h)
